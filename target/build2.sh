...
SOURCE_DIRS=(
    ...
    "../encryption" # Assuming fileEncryption.h is in ../encryption
)
...
# Define common include directories for external libraries within MinGW-w64 environments.
# These paths are typical for MSYS2/MinGW-w64 installations.
# Adjust if your setup places OpenSSL/libsodium headers elsewhere.
COMMON_LIB_INCLUDE_DIRS=(
    "/usr/x86_64-w64-mingw32/include" # Common MinGW-w64 include path on Linux
    "/mingw64/include"                # Common MSYS2/MinGW-w64 include path
)
...
# Add all specified source directories to the include paths.
# This is useful if header files are co-located with source files.
...
# Add common external library include directories
echo "Adding common external library include paths:"
for dir in "${COMMON_LIB_INCLUDE_DIRS[@]}"; do
    if [ -d "$dir" ]; then
        INCLUDE_FLAGS+=" -I$dir"
        echo "- $dir"
    else
        echo "Warning: Common library include directory '$dir' not found. Skipping."
    fi
done
...
if [ $? -eq 0 ]; then
    ...
    echo "    For external libraries like OpenSSL, ensure they are correctly installed in your MinGW-w64 environment."
